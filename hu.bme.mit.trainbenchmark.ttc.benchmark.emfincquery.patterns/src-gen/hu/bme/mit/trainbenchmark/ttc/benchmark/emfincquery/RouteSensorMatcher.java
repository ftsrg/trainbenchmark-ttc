...
public class RouteSensorMatcher extends BaseMatcher<RouteSensorMatch> {
  ...
  protected RouteSensorMatch tupleToMatch(final Tuple t) {
    try {
    	return RouteSensorMatch.newMatch((hu.bme.mit.trainbenchmark.ttc.railway.Sensor) t.get(POSITION_SENSOR));
  ...
  protected RouteSensorMatch arrayToMatch(final Object[] match) {
    try {
    	return RouteSensorMatch.newMatch((hu.bme.mit.trainbenchmark.ttc.railway.Sensor) match[POSITION_SENSOR]);
    } catch(ClassCastException e) {
  ...
  protected RouteSensorMatch arrayToMatchMutable(final Object[] match) {
    try {
    	return RouteSensorMatch.newMutableMatch((hu.bme.mit.trainbenchmark.ttc.railway.Sensor) match[POSITION_SENSOR]);
    } catch(ClassCastException e) {
    	LOGGER.error("Element(s) in array not properly typed!",e);
    	return null;
    }
