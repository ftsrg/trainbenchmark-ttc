...
public abstract class SemaphoreNeighborProcessor implements IMatchProcessor<SemaphoreNeighborMatch> {
   ...
   * @param pRoute3 the value of pattern parameter route3 in the currently processed match
   ...
   * 
   */
  public abstract void process(final Semaphore pSemaphore, final Route pRoute1, final Route pRoute3, final Sensor pSensor1, final Sensor pSensor2, final TrackElement pTe1, final TrackElement pTe2);
  
  @Override
  public void process(final SemaphoreNeighborMatch match) {
    process(match.getSemaphore(), match.getRoute1(), match.getRoute3(), match.getSensor1(), match.getSensor2(), match.getTe1(), match.getTe2());
  }
}
