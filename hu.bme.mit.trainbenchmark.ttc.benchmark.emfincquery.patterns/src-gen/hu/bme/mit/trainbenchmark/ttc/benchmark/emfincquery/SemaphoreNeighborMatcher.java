...
public class SemaphoreNeighborMatcher extends BaseMatcher<SemaphoreNeighborMatch> {
  ...
  protected SemaphoreNeighborMatch tupleToMatch(final Tuple t) {
    try {
    	return SemaphoreNeighborMatch.newMatch((hu.bme.mit.trainbenchmark.ttc.railway.Route) t.get(POSITION_ROUTE1));
  ...
  protected SemaphoreNeighborMatch arrayToMatch(final Object[] match) {
    try {
    	return SemaphoreNeighborMatch.newMatch((hu.bme.mit.trainbenchmark.ttc.railway.Route) match[POSITION_ROUTE1]);
    } catch(ClassCastException e) {
  ...
  protected SemaphoreNeighborMatch arrayToMatchMutable(final Object[] match) {
    try {
    	return SemaphoreNeighborMatch.newMutableMatch((hu.bme.mit.trainbenchmark.ttc.railway.Route) match[POSITION_ROUTE1]);
    } catch(ClassCastException e) {
    	LOGGER.error("Element(s) in array not properly typed!",e);
    	return null;
    }
